...
## 2026-03-22 - Single-Pass Map Replacements
**Learning:** In `internal/geocodebook/loader.go`, iterating over a map (like `resolved`) and sequentially calling `strings.ReplaceAll(sql, key, value)` causes intermediate O(N) memory allocations for each replacement step. Additionally, because map iteration order is random in Go, this approach can lead to unpredictable cascading replacements (where a previously replaced value contains a placeholder that is subsequently replaced by a later iteration).
**Action:** When performing multiple string replacements based on a dynamic map, always construct a `[]string` slice containing the keys and values, and apply them in a single pass using `strings.NewReplacer(replacements...).Replace(str)`. This avoids intermediate allocations and ensures all substitutions are evaluated simultaneously, preventing unintended cascading side effects.
