...
## 2026-03-24 - Zero-Allocation Dynamic String Replacement
**Learning:** In `internal/geocodebook/loader.go`, the `RenderPattern` function previously used a `for` loop over a map to sequentially call `strings.ReplaceAll` on a SQL template string. This approach caused O(N) intermediate string allocations and could lead to non-deterministic cascading replacements due to Go's random map iteration order.
**Action:** When replacing multiple dynamic variables in a string (e.g., from a map), avoid using a `for` loop with sequential `strings.ReplaceAll` calls. Instead, collect the key-value pairs into a `[]string` slice and perform a single-pass replacement using `strings.NewReplacer(replacements...).Replace(str)`. This prevents O(N) allocations and guarantees predictable replacement behavior.
