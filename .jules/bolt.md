...
## 2026-03-22 - Fast Zero-Allocation HTML Tag Stripping
**Learning:** Parsing and stripping HTML tags using regular expressions (`regexp.MustCompile`) like `(?is)<script[^>]*>` is inherently slow and allocation-heavy. Furthermore, attempting to manually strip tags using `strings.ToLower` for case-insensitive matching inside a loop creates severe O(N*M) memory allocations (creating new sub-strings continually) and risks Unicode byte-offset misalignment, which can corrupt strings.
**Action:** Replace regex-based HTML parsing with a manual `strings.Builder` iteration. To match specific tags case-insensitively, implement an allocation-free `indexIgnoreCase` helper (which checks prefixes byte-by-byte instead of casting the whole string to lowercase). Additionally, ensure robust tag boundary checking using an `isSpaceOrGreater` helper to avoid matching partial substrings (like matching `<scripting>` when looking for `<script>`).
