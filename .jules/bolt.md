...
## 2026-04-14 - Map Iteration and Cascading ReplaceAll
**Learning:** Replacing multiple dynamic variables in a string (e.g., from a map) using a `for` loop with sequential `strings.ReplaceAll` calls causes O(N) intermediate memory allocations and can lead to unpredictable cascading replacements due to random map iteration order.
