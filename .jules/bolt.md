...
## 2026-03-24 - Zero-Allocation Tag Extraction
**Learning:** Extracting content between simple, static XML/HTML tags (e.g., `<think>...</think>`) and removing the tags from the document using regular expressions (`regexp.MustCompile` and `FindStringSubmatch`/`ReplaceAllString`) causes significant memory allocation and execution time overhead. Regex engines require heap allocations for matches and replacements on every invocation.
**Action:** For simple static tag extraction or stripping, replace regular expressions with manual parsing using `strings.Index` to locate boundaries and `strings.Builder` to construct the modified text. This eliminates regex engine overhead and allows for near-zero allocation parsing, reducing execution time by >30x.
