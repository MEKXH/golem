...
## 2023-10-25 - Zero-Allocation Extraction of HTML/XML Blocks
**Learning:** Extracting text bounded by simple static tags (like `<think>...</think>`) using regular expressions (e.g., `regexp.MustCompile`) causes significant CPU and memory allocation overhead. A regex requires memory allocations to store its execution state and multiple passes over the input string during `FindStringSubmatch` and `ReplaceAllString`.
**Action:** When extracting content between simple static boundaries, avoid regular expressions. Use `strings.Index` to locate the exact string boundaries and perform O(1) string slicing (`content[start:end]`) to extract the inner block. Use a `strings.Builder` with pre-allocated capacity to quickly filter out the tags from the main string without any intermediate memory allocations. Ensure manual unclosed tag handling faithfully mirrors ungreedy regex match patterns.
