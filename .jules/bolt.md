...
## 2026-04-30 - Zero-Allocation Substring Extraction
**Learning:** Extracting content between simple, static XML/HTML tags (e.g., `<think>` blocks from LLM outputs) using `regexp.MustCompile` is heavily reliant on the regex engine, causing non-trivial execution overhead. Furthermore, `FindStringSubmatch` and `ReplaceAllString` cause multiple large memory allocations depending on the string size.
**Action:** Avoid using regular expressions for simple tag extraction. Instead, use `strings.Index` to find the start and end boundaries, extract the content using string slicing (`content[start:end]`), and reconstruct the remaining text using `strings.Builder`. This eliminates regex engine overhead and allows for near zero-allocation parsing, speeding up execution significantly (e.g., from ~6350 ns/op to ~140 ns/op).
