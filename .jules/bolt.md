...
## 2026-03-24 - Zero-Allocation Tag Parsing
**Learning:** Extracting text from static XML/HTML tags (e.g., `<think>`) using regular expressions (`regexp.MustCompile` and `ReplaceAllString`) incurs significant state machine overhead and causes multiple memory allocations. For high-frequency extraction, this creates measurable performance degradation compared to manual string parsing.
**Action:** When extracting or removing simple, static tags from strings, replace regular expressions with a manual pass using `strings.Index` to locate boundaries. Combined with `strings.Builder` (and pre-allocating using `.Grow()`) for constructing the remaining string, this eliminates regex overhead, achieves zero-allocation content extraction, and drastically reduces allocations for the surrounding string content.
