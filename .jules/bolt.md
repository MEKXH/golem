...
## 2025-05-18 - Optimized SplitThink in internal/render
**Learning:** Extracting content between simple XML/HTML tags (e.g., `<think>`) from LLM outputs using regular expressions (`regexp.MustCompile`) introduces significant engine overhead and intermediate string allocations. Our regex approach (`(?s)<think>(.*?)</think>`) was slow.
**Action:** Replaced `regexp` with `strings.Index` to find boundaries and extract content using simple string slicing. Implemented a fallback `strings.Builder` approach for handling multiple tags (which is rare but required for perfect backwards compatibility). The new approach is over 60x faster (8217 ns/op to 132.3 ns/op) and avoids regex parsing overhead completely. This pattern (avoiding regex for simple tag extraction) should be applied whenever parsing LLM output blocks.
