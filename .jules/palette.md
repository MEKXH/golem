...
## 2026-04-19 - Dynamic Log Auto-Scroll Accessibility
**Learning:** For dynamic interfaces that continuously update with new entries (such as chat timelines or live logs), newly added content may be rendered correctly but isn't automatically tracked by screen readers, making it difficult for assistive technology users to follow along. Furthermore, merely adding elements can cause the log to grow beyond the viewport without adjusting the scroll position.
**Action:** When creating live log components (e.g., chat timelines), add `role="log"` and `aria-live="polite"` to the container so that screen readers announce new incoming entries. Additionally, implement an auto-scrolling watch on the data source (`watch(() => entries.length, ..., { flush: 'post' })`) to keep the user's viewport automatically pinned to the newest entries, and bind `:aria-busy="isSending"` to indicate when the UI is waiting for new content.
