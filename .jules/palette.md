...
## 2026-04-18 - Chat Auto-Scrolling and Accessibility
**Learning:** For dynamic chat or log interfaces in Vue, implementing auto-scrolling can be tricky without redundant `await nextTick()` calls. Additionally, new content must be explicitly announced to screen readers.
**Action:** Bind a `ref` to the scrollable container and watch the data source length with `{ flush: 'post' }` to set `scrollTop = scrollHeight`. To ensure accessibility, apply `role="log"` and `aria-live="polite"` (and `aria-busy="true"` during loading states) to the container so screen readers actively announce new content.
