...
## 2026-04-20 - Dynamic Chat Interface Accessibility and Auto-scroll
**Learning:** Dynamic chat or log interfaces in Vue need explicit ARIA attributes (`role="log"`, `aria-live="polite"`, and `aria-busy`) so screen readers announce new content. Additionally, users expect auto-scrolling to new messages. Simply waiting for the next tick can be unreliable for auto-scrolling when lists update rapidly.
**Action:** For dynamic lists, apply `role="log"` and `aria-live="polite"` to the container. Implement auto-scrolling by watching the array length (and any loading state variables) with `{ flush: 'post' }` to reliably set `scrollTop = scrollHeight` after DOM updates.
