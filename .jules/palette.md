...
## 2026-05-02 - Chat Log Auto-Scroll Accessibility
**Learning:** When implementing auto-scroll in dynamic chat logs using Vue watchers with `flush: post`, screen readers may miss the new content if the container is not explicitly marked as a live region.
**Action:** Add `role="log"` and `aria-live="polite"` (and optionally `:aria-busy="isLoading"`) to chat timeline containers so assistive technologies correctly announce new messages as they are appended.
