...
## 2026-04-18 - Chat Auto-scroll Accessibility
**Learning:** For dynamic chat or log interfaces in Vue, implementing auto-scrolling requires accessing the DOM element explicitly, and simply rendering entries is insufficient. Without `role="log"` and `aria-live="polite"`, screen readers will not announce new incoming messages correctly, and users won't know when a message arrives.
**Action:** Always implement auto-scrolling by binding a `ref` to the scrollable container, watching the data source length, and using `nextTick()` to set `scrollTop = scrollHeight`. To ensure accessibility, apply `role="log"` and `aria-live="polite"` (and `aria-busy="true"` during loading states) to the container so screen readers actively announce new content.
