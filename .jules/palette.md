...
## 2026-04-21 - Auto-scroll and screen reader support for chat timeline
**Learning:** Applying `role="log"` and `aria-live="polite"` combined with `{ flush: 'post' }` for auto-scrolling in Vue chat interfaces makes dynamic content accessible and intuitive.
**Action:** Always bind a ref and watch data source length with `flush: 'post'` to set `scrollTop = scrollHeight` for dynamic chat/log interfaces.
