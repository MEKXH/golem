...
## 2026-04-18 - Chat Interface Accessibility and Scrolling
**Learning:** For dynamic chat or log interfaces in Vue, developers often use redundant `await nextTick()` calls when new data is added, and fail to announce new entries to screen reader users. The container must have a specific ARIA role and live region attribute.
**Action:** Implement auto-scrolling by binding a `ref` to the scrollable container and watching the data source length (and any loading state variables like `isSending` that append temporary messages) with `{ flush: 'post' }` to set `scrollTop = scrollHeight`. To ensure accessibility, apply `role="log"` and `aria-live="polite"` (and `aria-busy="true"` during loading states) to the container so screen readers actively announce new content.
