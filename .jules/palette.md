...
## 2026-04-16 - Dynamic Chat Timeline Auto-Scrolling and Accessibility
**Learning:** For dynamic chat or log interfaces in Vue, implementing auto-scrolling is necessary to keep the latest message in view. Additionally, without ARIA attributes, screen readers won't announce new content as it appears, leading to a poor experience for visually impaired users.
**Action:** Implement auto-scrolling by binding a `ref` to the scrollable container, watching the data source length and loading state, and using `nextTick()` to set `scrollTop = scrollHeight`. To ensure accessibility, apply `role="log"` and `aria-live="polite"` (and `aria-busy="true"` during loading states) to the container so screen readers actively announce new content.
