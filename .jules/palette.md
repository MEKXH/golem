...
## 2026-04-26 - Dynamic Chat Timeline Accessibility & Scrolling
**Learning:** Dynamic chat or log interfaces in Vue require both visual and non-visual feedback. Without explicit ARIA attributes (`role="log"`, `aria-live="polite"`, `aria-busy`), screen readers fail to announce new incoming entries. Furthermore, simply appending to an array does not auto-scroll the user, and using `await nextTick()` can cause race conditions or redundant renders.
**Action:** Always implement auto-scrolling by binding a `ref` to the scrollable container and watching the data source length (and loading states) with `{ flush: 'post' }` to set `scrollTop = scrollHeight`. To ensure accessibility, apply `role="log"` and `aria-live="polite"` (and `aria-busy="true"` during loading) to the container.
