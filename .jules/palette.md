...
## 2026-04-18 - Dynamic Chat Auto-scrolling and Accessibility
**Learning:** When adding new items to a chat log or timeline in Vue, trying to scroll down immediately after updating the array might fail because the DOM hasn't updated yet. Also, dynamic chat streams need explicit ARIA tags so screen readers announce incoming messages.
**Action:** For dynamic chat or log interfaces in Vue, implement auto-scrolling by binding a `ref` to the scrollable container and watching the data source length with `{ flush: 'post' }` to set `scrollTop = scrollHeight` (avoiding redundant `await nextTick()` calls). To ensure accessibility, apply `role="log"` and `aria-live="polite"` (and `aria-busy="true"` during loading states) to the container so screen readers actively announce new content.
