<template>
  <form class="composer-panel" @submit.prevent="$emit('submit-prompt')">
    <textarea :value="modelValue" rows="3" :placeholder="consoleCopy.composer.placeholder" @input="$emit('update:modelValue', ($event.target as HTMLTextAreaElement).value)" @keydown.enter.exact.prevent="$emit('submit-prompt')"></textarea>
